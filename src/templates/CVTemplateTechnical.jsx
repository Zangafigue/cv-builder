...
export default function CVTemplateTechnical({ data }) {
  ...
  // Renderers so each column can honor the user's sectionsOrder.
  const sectionRenderers = {
    experience: () => experience.length === 0 ? null : (
    ...
    education: () => education.length === 0 ? null : (
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, borderBottom: `1px solid ${themeColor}40`, paddingBottom: '0.2rem', marginBottom: '1rem', fontWeight: 700, display: 'inline-block' }}>
          &gt; _education
        </h2>
    ...
    customSections: () => customSections.length === 0 ? null : (
      <React.Fragment>
    ...
    skills: () => skills.length === 0 ? null : (
      <div style={{ marginBottom: '2rem', padding: '1rem', backgroundColor: '#f8fafc', border: `1px solid ${themeColor}30`, borderRadius: '4px' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, marginBottom: '1rem', marginTop: 0, fontWeight: 700 }}>&gt; _stack</h2>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
          {skills.map(skill => (
            <span key={skill.id} style={{ backgroundColor: 'rgba(0,0,0,0.8)', color: '#6ee7b7', padding: '0.2rem 0.5rem', fontFamily: 'monospace', fontSize: '0.85rem', borderRadius: '2px' }}>
              {skill.name}
            </span>
          ))}
        </div>
    ...
    languages: () => languages.length === 0 ? null : (
      <div style={{ marginBottom: '2rem', padding: '1rem', backgroundColor: '#f8fafc', border: `1px solid ${themeColor}30`, borderRadius: '4px' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, marginBottom: '1rem', marginTop: 0, fontWeight: 700 }}>&gt; _langs</h2>
    ...
    certifications: () => certifications.length === 0 ? null : (
      <div style={{ marginBottom: '2rem', padding: '1rem', backgroundColor: '#f8fafc', border: `1px solid ${themeColor}30`, borderRadius: '4px' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, marginBottom: '1rem', marginTop: 0, fontWeight: 700 }}>&gt; _certs</h2>
    ...
    extracurricular: () => extracurricular.length === 0 ? null : (
      <div style={{ marginBottom: '2rem', padding: '1rem', backgroundColor: '#f8fafc', border: `1px solid ${themeColor}30`, borderRadius: '4px' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, marginBottom: '1rem', marginTop: 0, fontWeight: 700 }}>&gt; _extra</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontFamily: 'monospace', fontSize: '0.8rem' }}>
          {extracurricular.map((item, i) => (
            <div key={i}>- {typeof item === 'string' ? item : (item.name || item.description)}</div>
          ))}
        </div>
      </div>
    ),
    interests: () => interests.length === 0 ? null : (
      <div style={{ padding: '1rem', backgroundColor: '#f8fafc', border: `1px solid ${themeColor}30`, borderRadius: '4px' }}>
        <h2 style={{ fontSize: '1.2rem', color: themeColor, marginBottom: '1rem', marginTop: 0, fontWeight: 700 }}>&gt; _hobby</h2>
  ...
  const MAIN_IDS = ['experience', 'projects', 'education', 'customSections'];
  const SIDEBAR_IDS = ['skills', 'languages', 'certifications', 'extracurricular', 'interests'];
  const mainOrder = sectionsOrder.filter(id => MAIN_IDS.includes(id));
  const sidebarOrder = sectionsOrder.filter(id => SIDEBAR_IDS.includes(id));
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '2rem', borderBottom: `2px dashed ${themeColor}60`, paddingBottom: '1rem' }}>
        <CvPhoto photo={personalInfo.photo} settings={{ ...personalInfo.photoSettings, shape: 'square' }} size={90} style={{ marginBottom: '1rem', border: `2px solid ${themeColor}` }} />
